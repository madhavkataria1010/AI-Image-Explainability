...
batch_size = 64
num_epochs = 100
save_best_path = './models/efficientvit_b0_best.pt'
save_path = './models/efficientvit_b0_final.pt'
log_file = './logs/efficientvit_b0.json'
patience = 10
...
"""
    Fine-tuned Efficient Vit b0 model on our training dataset with custom head to meet our requirements of binary classification
"""

model = create_model('efficientvit_b0', pretrained=True)

num_features = model.head.classifier[0].in_features  
model.head.classifier = nn.Sequential(
    ...
    nn.LayerNorm((64,)),                    
    nn.Hardswish(),                           
    nn.Dropout(p=0.5),   nn.Linear(64,2),   )     
