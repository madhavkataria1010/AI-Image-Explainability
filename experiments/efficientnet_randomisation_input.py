...
batch_size = 64
num_epochs = 100
save_best_path = './models/efficientnet_randLayer_b0_best.pt'
save_path = './models/efficientnet_randLayer_b0_final.pt'
log_file = './logs/efficientnet_randLayer_b0.json'
patience = 10
...
"""
    Fine-tuned Efficient Net b0 model on our training dataset with random input initalisation to make roboust against adversarial attack
"""

class RandomImageDefense(nn.Module):
    def __init__(self, base_model, resize_range=(32, 34), padding_limit=2):
        super().__init__()
        self.base_model = base_model
        self.resize_min, self.resize_max = resize_range
        self.padding_limit = padding_limit
        self.dropout = nn.Dropout(p=0.5)
    
    def random_resize(self, x):
        resize_size = np.random.randint(self.resize_min, self.resize_max)
    ...
    def random_pad(self, x):
        current_size = x.shape[2]
        pad_total = current_size - 32

        if pad_total <= 0:
            return x

        max_pad = min(pad_total, self.padding_limit)
        
        pad_left = np.random.randint(0, max_pad + 1)
        pad_top = np.random.randint(0, max_pad + 1)
...
model = RandomImageDefense(model)
